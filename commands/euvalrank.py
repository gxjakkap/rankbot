...
class euvalrank(BaseCommand):
    def __init__(self):
        description = "Get VALORANT rank for requested player, EU only"
    ...
    async def handle(self, params, message, client):
        ...
        ign = ''.join(ignl)
        tag = ''.join(tagl)
        r = requests.get('https://api.henrikdev.xyz/valorant/v1/mmr/eu/'+ign+'/'+tag)
        r.encoding = 'utf-8'
        ans = r.json()
        try:
            x  = ans['data']

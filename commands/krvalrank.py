...
class krvalrank(BaseCommand):
    def __init__(self):
        description = "Get VALORANT rank for requested player, KR only"
    ...
    async def handle(self, params, message, client):
        ...
        ign = ''.join(ignl)
        tag = ''.join(tagl)
        r = requests.get('https://api.henrikdev.xyz/valorant/v1/mmr/kr/'+ign+'/'+tag)
        r.encoding = 'utf-8'
        ans = r.json()
        try:
            x  = ans['data']
